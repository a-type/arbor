...
import { NO_NEGATIVE } from '../util/control.js';

const anchoredNumberRE = /^-?[0-9.]+(?:[a-z]+|%)?$/;
const numberRE = /-?[0-9.]+(?:[a-z]+|%)?/;
const spacingMultiplyRE =
	/var\(--spacing(?:-[\w-]+)?\)\s*\*\s*(-?[0-9.]+(?:[a-z]+|%)?)/;
...
function negateMathFunction(value: string) {
	...
	if (match) {
		const [fnBody, rest] =
			getStringComponent(`(${match[2]})${match[3]}`, '(', ')', ' ') ?? [];
		if (fnBody) {
...
function negateFunctionBody(value: string) {
	...
	if (match) {
		...
		if (fnBody) {
			const body =
				anchoredNumberRE.test(fnBody.slice(1, -1)) ?
					fnBody.replace(numberRE, (i) =>
						i.startsWith('-') ? i.slice(1) : `-${i}`,
					)
				:	`(calc(${fnBody} * -1))`;
...
export const variantNegative: Variant<Theme> = {
	...
	match(matcher) {
		...
		return {
			...
			body: (body) => {
				if (body.some((v) => v[0] === NO_NEGATIVE)) return;
				...
				for (const v of body) {
					...
					if (meta && toArray(meta).includes(NO_NEGATIVE)) continue;
