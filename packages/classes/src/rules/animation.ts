...
export const animations: Rule<Theme>[] = [
	...
	[
		...
		([, d], { theme }) => ({
			'animation-duration':
				theme.duration?.[d || 'DEFAULT'] ?? h.bracket.cssvar.time(d),
		}),
	...
	[
		...
		([, d], { theme }) => ({
			'animation-timing-function':
				theme.easing?.[d || 'DEFAULT'] ?? h.bracket.cssvar(d),
		}),
	...
	[
		...
		([, d]) => ({
			'animation-iteration-count': h.bracket.cssvar(d) ?? d.replace(/-/g, ','),
		}),
		{
			autocomplete: [
				'animate-(iteration|count|iteration-count)',
	...
	[
		...
		([, t, d]) =>
			['paused', 'running', ...[t ? globalKeywords : []]].includes(d) ?
