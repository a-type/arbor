...
const defaultShadowBlurCss = (step: number, $: GlobalTokens) => css`
	calc(${[$.shadow.blur, '0.5']} * ${[
		$.space.baseSize,
		'0.5rem',
	]} * 0.25 * pow(2, ${step} - 1))
`;
const defaultShadowSpreadCss = (step: number, $: GlobalTokens) => css`
	calc(${[$.shadow.spread, '0.5']} * 1px)
`;
const defaultShadowColorCss = (step: number, $: GlobalTokens) => css`
	${$.shadow.color}
`;
